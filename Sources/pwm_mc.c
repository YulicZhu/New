...
void ask_duty(){
	uint16_t new_CBDR,Vol_AR1,Vol_AR2;
	Vol_AR1=ADC.CDR[AR1].B.CDATA*5000/0x3FF;
	new_CBDR=(uint16_t)((0.2+Vol_AR1*0.001*0.12)*Timecycle);
	
	if((new_CBDR-DUTY_REGISTER)>0.01*Timecycle||(new_CBDR-DUTY_REGISTER)<-0.01*Timecycle){
		DUTY_REGISTER= new_CBDR;//duty=0.2~0.8
		DIS_NUM = (0.2+Vol_AR1*0.001*0.12)*1000;
	}
}
